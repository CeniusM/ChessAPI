...
#include "MovesFromSquare.h"
#include "Directions.h"

class MovesFromSquare
{
public:
    static const int InvalidMove = 2147483647;
    static int KingMoves[64][8];
    static int KnightMoves[64][8];

    ///<summary> 
    /// Square, direction, the moves in the direction.
    /// Doesn't count the square its on and the line ends at ivalidMove
    /// </summary>
    static int SlidingpieceMoves[64][8][7];
    static void Init()
    {
        if (isInit)
            return;
    ...
    static bool IsInBounds(int place)
    {
        return (place < 64 && place > -1);
    }
